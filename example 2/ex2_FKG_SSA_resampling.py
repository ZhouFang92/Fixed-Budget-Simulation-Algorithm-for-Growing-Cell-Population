...
resampling_time = [t for t in np.arange(0, T+1, 3)]
if resampling_time[-1] != T:
    resampling_time.append(T)
num = 10000 #50000
...
for i in range(len(resampling_time)-1):
    print("Computing the", i+1, "th period. t = ", resampling_time[i], "to", resampling_time[i+1])
    if i == 0:
        dist, ESS, weight_all, state_all = FGK_SSA_one_period(sample_size=num, time_period=resampling_time[1], mu_total=10)
    else:
        init_particles = {'state_list': state_all, 'weight_list': weight_all}
        mu_total = dist.sum()
        print("mu_total:", mu_total)
        dist, ESS, weight_all, state_all = FGK_SSA_one_period(sample_size=num, time_period=resampling_time[i+1]-resampling_time[i], mu_total=mu_total, init_particles=init_particles)
    ESS_list.append(ESS)
    print("ESS:", ESS, "\n")
